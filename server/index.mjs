...
const allowedOrigins = new Set([`http://localhost:${port}`, `http://127.0.0.1:${port}`, 'http://localhost:4317', 'http://127.0.0.1:4317']);
...
server.listen(port, '127.0.0.1', () => console.log(`Nextstep running at http://127.0.0.1:${port}\nDatabase: ${database}`));
let closing = false;
for (const signal of ['SIGTERM', 'SIGINT']) process.on(signal, () => { if (closing) return; closing = true; server.close(() => { store.close(); process.exit(0); }); server.closeIdleConnections(); });
