...
import { connectionConfig, setupInstructions } from './agent-instructions.mjs';
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/agent/setup' && req.method === 'GET') return json(res,200,{instructions:setupInstructions(store.agentStatus().schedule)});
