...
export function setupInstructions(schedule) {
...
1. Call nextstep_get_context and confirm the database is the one connected to Nextstep. If the tool is unavailable, help me finish the MCP connection first. Do not create a replacement database or use an unrelated checkout.
...
3. Read the saved schedule from context. ${schedule?.cadence ? `The requested cadence is ${schedule.cadence}, timezone ${schedule.timezone}.` : 'Ask me for a cadence and timezone and have me save them in Nextstep before scheduling.'} Use the native local scheduled-task tools in this desktop agent to create or update the task. Inspect existing tasks first; reuse the registered task ID or a matching task for this exact database. Never create a duplicate. Do not edit scheduler configuration files or invent an API. If native scheduling is unavailable, explain the required local desktop setup and leave the task unregistered.
4. The scheduled task must run locally with Nextstep MCP available, and always point to this same absolute database path even if the agent uses a worktree. Use the recurring instructions below without copying today's preferences or board contents into the task. Confirm creation using the agent's task tool, then call nextstep_register_schedule with the actual task ID and the saved schedule version. This records the agent's report; it does not create the schedule itself.
5. Test one search when I am ready, check the imported IDs and report in Nextstep, and explain that local scheduled work requires the computer awake and the agent app running. Review any tool approvals in the agent. The MCP helper can operate when the Nextstep window is closed.
