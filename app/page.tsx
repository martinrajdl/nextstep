...
import { ArrowDownToLine, ArrowUpRight, CheckCheck, Columns3, Eye, EyeOff, Layers3, List, Plus, RefreshCw, Search, UserRound, X } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    const focus = () => { if (!busyRef.current && !editor) void refresh(); };
    window.addEventListener('focus', focus); return () => window.removeEventListener('focus',focus);
  }, [editor,refresh]);
  useEffect(() => {
    function key(event: KeyboardEvent) {
      if (event.metaKey || event.ctrlKey || event.altKey || editor || loading || loadError) return;
