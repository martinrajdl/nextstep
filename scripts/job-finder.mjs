...
import { openStore, STAGES } from '../server/store.mjs';
...
try {
  ...
  if (command === 'context' && !filename) {
    ...
    try {
      ...
      console.log(JSON.stringify({database,readAt:new Date().toISOString(),byStage:Object.fromEntries(STAGES.map(stage => [stage,records.filter(job => job.stage === stage && !job.removedAt)])),removed:records.filter(job => job.removedAt)},null,2));
    } finally { db.close(); }
    ...
    throw new Error('Usage: sh scripts/job-finder.sh context | import /absolute/path/jobs.json [--dry-run]. Use - to read JSON from stdin.');
  }
} catch (error) {
