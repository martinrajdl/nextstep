...
export default function AgentConnection({onClose}: Props) {
  ...
  return <><Sheet open onOpenChange={open => { if (!open) close(); }}><SheetContent className="opportunity-sheet agent-sheet">
    ...
    <div className="editor-form"><div className="editor-scroll">
      {!state ? <output>{error ? 'Could not load agent setup.' : 'Loading agent setup…'}</output> : <>
        <form onSubmit={save} aria-label="Agent setup"><fieldset disabled={busy}>
          <div className="field"><label htmlFor="agent-provider">Choose your desktop agent</label><select id="agent-provider" required value={draft.provider} onChange={event => setDraft({...draft,provider:event.target.value})}><option value="">Choose an agent</option><option value="codex">Codex / ChatGPT desktop</option><option value="claude-code">Claude Code Desktop</option></select><p className="agent-hint">Use a local desktop task so it can reach your SQLite database.</p></div>
          <div className="field"><label htmlFor="agent-cadence">When should it search?</label><input id="agent-cadence" required maxLength={300} value={draft.cadence} placeholder="Describe the days and time you want" onChange={event => setDraft({...draft,cadence:event.target.value})}/></div>
