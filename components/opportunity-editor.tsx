  ...
  return <><Sheet open onOpenChange={open => { if (!open) close(); }}><SheetContent className="opportunity-sheet" showCloseButton>
    ...
    <form onSubmit={save} className="editor-form" aria-label="Opportunity details">
      <div className="editor-scroll"><fieldset disabled={busy}>
        <div className="field"><label htmlFor="company">Company <span className="required-mark">*</span></label><Input id="company" required maxLength={160} placeholder="e.g. Linear" value={draft.company} onChange={e => field('company',e.target.value)}/></div>
        <div className="field"><label htmlFor="role">Role</label><Input id="role" maxLength={200} placeholder="e.g. Senior Product Engineer" value={draft.role} onChange={e => field('role',e.target.value)}/></div>
        ...
        <div className="field-row"><div className="field"><label htmlFor="location">Location / work setup</label><Input id="location" maxLength={200} placeholder="Remote · Europe" value={draft.location} onChange={e => field('location',e.target.value)}/></div><div className="field"><label htmlFor="salary">Compensation</label><Input id="salary" maxLength={160} placeholder="e.g. €120–150k / year" value={draft.salary} onChange={e => field('salary',e.target.value)}/></div></div>
        ...
        <div className="field"><label htmlFor="nextStep">Next step</label><Input id="nextStep" maxLength={500} placeholder="e.g. Send portfolio to the hiring manager" value={draft.nextStep} onChange={e => field('nextStep',e.target.value)}/></div>
