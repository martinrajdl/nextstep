...
test('company-only prospects, optional fields, and edits survive reopening the database', t => {
  ...
  const edited = store.update(job.id,{version:job.version,role:'Product Engineer',notes:'First call\nAsk about the team',followUp:'2026-10-02',stage:'screening',url:'https://example.com/jobs'});
...
for (const schemaVersion of [1,2]) test(`upgrading schema ${schemaVersion} preserves jobs and history and supports Uninterested`, t => {
  ...
  legacy.exec(`PRAGMA foreign_keys = ON;
    CREATE TABLE opportunities (
      ...
      stage TEXT NOT NULL CHECK(stage IN ('prospect',${schemaVersion === 2 ? "'interested'," : ''}'applied','screening','interview','final','offer','accepted','rejected','withdrawn')),
...
test('finder skips known and removed jobs, preserves user choices, and prepends ranked new prospects', t => {
  const {store} = database(t);
  const oldA = store.create({company:'Existing A',role:'Product Engineer'});
  const oldB = store.create({company:'Existing B',role:'Frontend Engineer'});
  const declined = store.create({company:'Declined',role:'Backend Engineer',stage:'uninterested',notes:'Backend-first',url:'https://jobs.ashbyhq.com/declined/10000000-0000-0000-0000-000000000001'});
  const interested = store.create({company:'Selected',role:'Senior Product Engineer',stage:'interested',priority:'high',notes:'My notes',url:'https://jobs.ashbyhq.com/selected/10000000-0000-0000-0000-000000000002'});
  const applied = store.create({company:'Applied',role:'Frontend Engineer',stage:'applied',url:'https://jobs.lever.co/applied/10000000-0000-0000-0000-000000000003'});
  const removed = store.create({company:'Removed',role:'Engineer',url:'https://example.com/jobs/removed'});
  store.remove(removed.id,{version:removed.version});
  const before = store.list(); const removedBefore = store.get(removed.id,true);
  const candidates = [
    {company:'New A',role:'Product Engineer',url:'https://example.com/jobs/new-a',stage:'interested',priority:'high'},
    {company:'Declined',role:'Renamed backend role',url:'https://declined.example/careers?ashby_jid=10000000-0000-0000-0000-000000000001&utm_source=test'},
    {company:'Selected',role:'Senior - Product Engineer',url:'https://example.com/alternate'},
    ...
    {company:'Removed',role:'Engineer',url:'https://example.com/jobs/removed?utm_source=test'},
    {company:'New B',role:'Frontend Engineer',url:'https://example.com/jobs/new-b'},
    {company:'New A',role:'Product Engineer',url:'https://example.com/jobs/new-a?utm_source=duplicate'},
  ];
  ...
  const newest = store.importProspects([{company:'Next run',role:'Engineer',url:'https://example.com/jobs/next-run'}]);
...
test('finder respects company exclusions while allowing different roles and rejects invalid batches atomically', t => {
  ...
  store.create({company:'Mixed company',role:'Backend Engineer',stage:'uninterested'});
  const result = store.importProspects([
    {company:'Excluded company',role:'Frontend Engineer',url:'https://example.com/excluded'},
    {company:'Mixed company',role:'Frontend Engineer',url:'https://example.com/mixed'},
  ]);
  ...
  assert.throws(() => store.importProspects([
    {company:'Valid',role:'Engineer',url:'https://example.com/valid'},
    {company:'Invalid',role:'Engineer',url:'javascript:alert(1)'},
  ]));
...
test('finder CLI reads the same database and imports safely alongside an open app connection', t => {
  const {store,file} = database(t);
  const existing = store.create({company:'Already applied',role:'Engineer',stage:'applied'});
  ...
  writeFileSync(input,JSON.stringify([{company:'CLI prospect',role:'Engineer',url:'https://example.com/cli'}]));
